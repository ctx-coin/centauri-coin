...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 43229
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 43228
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 33812
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 33813
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 43229
	%{_sbindir}/semanage port -d -p tcp 43228
	%{_sbindir}/semanage port -d -p tcp 33812
	%{_sbindir}/semanage port -d -p tcp 33813
	for selinuxvariant in %{selinux_variants}; do
